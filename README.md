...
| Port 8080 still answers but no window seems to be running it | The server outlived its terminal (closing a window with the X does not stop it). `.\scripts\stop.ps1` finds it by port and stops it; `-WhatIf` just shows what is there |
